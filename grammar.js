module.exports = grammar({
  ...
  rules: {
    ...
    _statement: ($) => choice($.declaration, $.comment),
    ...
    constructor: ($) =>
      prec.left(seq($.constructor_name, optional($.constructor_tag))),

    constructor_name: ($) => choice("_", $.identifier),
    ...
    field_parameter_def: ($) =>
      seq("{", repeat1(seq($.identifier, ":", $.type_identifier)), "}"),

    field_curly_expr_def: ($) => seq("{", $._expression, "}"),

    field_named_def: ($) => seq($.identifier, ":", $._field_value),
    ...
    field_anonymous_def: ($) =>
      prec.right(
        "sequence",
        seq(optional("^"), "[", repeat($.field_definition), "]"),
      ),
    ...
    conditional_expression: ($) =>
      prec.right("conditional", seq($._expression, "?", $._type_expression)),
    ...
    _simple_type_expr: ($) =>
      choice($.identifier, "Any", $._number, $.parenthesized_type_expression),
    ...
    bit_selection_expression: ($) => seq($._expression, ".", $._number),
    ...
    type_application: ($) =>
      prec.right(
        "application",
        seq($.identifier, repeat1($._type_app_argument)),
      ),

    parenthesized_type_application: ($) =>
      seq("(", $.identifier, repeat1($._type_expression), ")"),
    ...
    builtin_operator: ($) => choice("##", "#<", "#<=", "lenBits"),

    builtins_zero_args: ($) => "#",

    _ref_expression: ($) => prec("atom", choice($.identifier, $._number)),

    parenthesized_expression: ($) => seq("(", $._expression, ")"),

    parenthesized_type_expression: ($) => seq("(", $._type_expression, ")"),
    ...
    type_identifier: ($) => choice("Type", "#", $.identifier),
